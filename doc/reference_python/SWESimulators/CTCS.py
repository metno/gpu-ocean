...
class CTCSDataCL:
    ...
    def download(self, cl_queue):
        ...
        eta1 = np.empty((self.ny+2, self.nx+2), dtype=np.float32, order='C')
        u1 = np.empty((self.ny+2, self.nx+1), dtype=np.float32, order='C')
        v1 = np.empty((self.ny+1, self.nx+2), dtype=np.float32, order='C')
        
        #Copy data from device to host
        cl.enqueue_copy(cl_queue, eta1, self.eta1)
        cl.enqueue_copy(cl_queue, u1, self.u1)
        cl.enqueue_copy(cl_queue, v1, self.v1)
...
class CTCS:
    ...
    def __init__(self, \
        ...
        self.cl_data = CTCSDataCL(self.cl_ctx, h0, eta0, u0, v0)
