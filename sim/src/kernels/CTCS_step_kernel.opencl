...
__kernel void ctcsStepKernel(
        ...
        // Read and write Data
        __global float* eta0_ptr_, const int eta0_pitch_, //eta^n-1 (also used as output, that is eta^n+1)
        __global float* U0_ptr_, const int U0_pitch_, // U^n-1, also output, U^n+1
        __global float* V0_ptr_, const int V0_pitch_, // V^n-1, also output V^n+1

        // Read data
        __global float* H_ptr_, const int H_pitch_,
        __global float* eta1_ptr_, const int eta1_pitch_, // eta^n
        __global float* U1_ptr_, const int U1_pitch_, // U^n
        __global float* V1_ptr_, const int V1_pitch_, // V^n 
    
        // Wind stress parameters
        const float wind_stress_t_) {
        
    __local float H_shared[block_height+2][block_width+2];
    __local float eta1_shared[block_height+2][block_width+2];
    ...
    
	{
        ...
        const float f_v_0 = f_ + beta_ * ((get_local_size(1) * get_group_id(1) + get_local_id(1))+0.5f-y_zero_reference_cell_ + 0.5f)*dy_;
        const float f_v_m = f_ + beta_ * ((get_local_size(1) * get_group_id(1) + get_local_id(1))-0.5f-y_zero_reference_cell_ + 0.5f)*dy_;
	...
	// Find next V:
    {
        ...
        // Coriolis at U positions: 
        const float f_u_0 = f_ + beta_ * ((get_local_size(1) * get_group_id(1) + get_local_id(1)+1)-0.5f-y_zero_reference_cell_)*dy_;
        const float f_u_p = f_ + beta_ * ((get_local_size(1) * get_group_id(1) + get_local_id(1)+1)+0.5f-y_zero_reference_cell_)*dy_;	
