...
float3 computeFFaceFlux(int i, int j,
    ...
    const float2 Rp = (float2)(up - 0.5f*Qx[0][j][i+1], vp - 0.5f*Qx[1][j][i+1]);
    const float2 Rm = (float2)(um + 0.5f*Qx[0][j][i  ], vm + 0.5f*Qx[1][j][i  ]);
...
float3 computeGFaceFlux(int i, int j,
    ...
    const float2 Rp = (float2)(up - 0.5f*Qy[0][j+1][i], vp - 0.5f*Qy[1][j+1][i]);
    const float2 Rm = (float2)(um + 0.5f*Qy[0][j  ][i], vm + 0.5f*Qy[1][j  ][i]);
    ...
    return (float3)(flux.x, flux.z, flux.y);
}
