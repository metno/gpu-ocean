...
__kernel void fblStepKernel(
    ...
    __local float U_shared[block_height+2][block_width+1];
    __local float V_shared[block_height+3][block_width+2];
    __local float eta_shared[block_height+2][block_width+2];

    //Index of thread within block
    const int tx_u = get_local_id(0);   // U has no ghost cells in x-dir
    const int tx = get_local_id(0) + 1; 
    const int ty = get_local_id(1) + 1;
    ...
    for (int j=get_local_id(1); j<block_height+2; j+=get_local_size(1)) {
        const int l = by + j;
        ...
        for (int i=get_local_id(0); i<block_width+2; i+=get_local_size(0)) {
            const int k = bx + i;
    ...
    for (int j=get_local_id(1); j<block_height+3; j+=get_local_size(1)) {
        const int l = by + j;
    ...
    for (int j=get_local_id(1); j<block_height+2; j+=get_local_size(1)) {
        ...
        //Compute the pointer to current row in the V array
        __global float* U_row = (__global float*) ((__global char*) U_ptr_ + U_pitch_*l);

        for (int i=get_local_id(0); i<block_width+1; i+=get_local_size(0)) {
            const int k = bx + i;
    ...
    for (int j=get_local_id(1)+1; j<block_height+2; j+=get_local_size(1)) {
        const unsigned int l = by + j;

        for (int i=get_local_id(0)+1; i<block_width+1; i+=get_local_size(0)) {
